...
export function SupportRequestCreatePage() {
  ...
  return <><PageHeader title="New Support Request" /><SupportRequestForm clients={clients.data?.items ?? []} projects={projects.data?.items ?? []} onSubmit={(values) => mutation.mutate(values)} disabled={mutation.isPending} /></>;
}

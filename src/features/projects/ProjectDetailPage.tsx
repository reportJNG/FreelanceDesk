...
export function ProjectDetailPage({ id }: { id: string }) {
  ...
  return <>
    <PageHeader title={project.name} action={<a className="button" href={`/projects/${id}/edit`}>Edit project</a>} />
    <div className="grid detail-grid">
      <div className="card"><h3>Status</h3><StatusBadge value={project.status} /><p>{project.description}</p></div>
      <div className="card"><h3>Financials</h3><div className="info-row"><span>Total</span><strong>{formatMoney(project.total_price)}</strong></div><div className="info-row"><span>Paid</span><strong>{formatMoney(project.paid_amount ?? 0)}</strong></div><div className="info-row"><span>Remaining</span><strong>{formatMoney(project.remaining_amount ?? 0)}</strong></div><div className="info-row"><span>Expenses</span><strong>{formatMoney(project.expenses)}</strong></div></div>
      <div className="card"><h3>Links</h3><p className="code-value">{project.production_url || "No live URL"}</p><CopyButton value={project.production_url} /></div>
      ...
      <div className="card"><h3>Notes</h3><p className="muted">{project.notes || "No notes"}</p></div>
    </div>
  </>;
}
